...
class AudioFeatureExtractor:
    ...
    def extract_compression_features(y, sr):
        ...
        try:
            ...
            features['estimated_bit_depth'] = np.log2(unique_values) if unique_values > 1 else 0
...
class AudioAnalyzer:
    ...
    def analyze_audio_file(self, audio_path, source_dir=None, save_spectrogram=False, 
                          spectrogram_dir=None, spectrogram_type='mel'):
        ...
        try:
            ...
            features = {
                ...
                'source_directory': source_dir if source_dir else audio_path.parent.name,
            ...
            if save_spectrogram and spectrogram_dir:
                ...
                features['spectrogram_path'] = str(spectrogram_path) if spectrogram_path else None
    ...
    def save_spectrogram(self, audio_path, output_dir, spectrogram_type='mel', 
                        figsize=(12, 8), dpi=300, include_colorbar=True):
        """
        ...
        try:
            ...
            if spectrogram_type.lower() == 'mel':
                ...
                img = librosa.display.specshow(C_dB, x_axis='time', y_axis='cqt_hz', 
                                             sr=sr)
            ...
            if include_colorbar:
                plt.colorbar(img, format='%+2.0f dB' if spectrogram_type in ['mel', 'stft', 'cqt'] else None)
...
def process_single_audio_file(args):
    ...
    if len(args) == 3:
        ...
        audio_file, is_ai, source_dir, save_spectrogram, spectrogram_dir, spectrogram_type = args
...
class AIAudioDetector:
    ...
    def __init__(self, base_dir=None, config_path=None):
        ...
        self.incremental_models = {
            ...
            'PassiveAggressive': PassiveAggressiveClassifier(**self.config['models']['incremental']['passive_aggressive'])
        }
        
        self.batch_models = {
            'RandomForest': RandomForestClassifier(**self.config['models']['batch']['random_forest']),
            'GradientBoosting': GradientBoostingClassifier(**self.config['models']['batch']['gradient_boosting'])
        }
    ...
    def extract_features_from_directory(self, audio_dir, is_ai_directory=True, max_workers=None,
                                      save_spectrograms=False, spectrogram_dir=None, 
                                      spectrogram_type='mel'):
        ...
        if save_spectrograms and spectrogram_dir:
            args_list = [(audio_file, is_ai_directory, source_name, save_spectrograms, 
                         spectrogram_dir, spectrogram_type) for audio_file in audio_files]
        else:
            args_list = [(audio_file, is_ai_directory, source_name) for audio_file in audio_files]
        ...
        if len(audio_files) > batch_threshold:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                future_to_file = {executor.submit(process_single_audio_file, args): args[0] 
                                 for args in args_list}
                
                with tqdm(total=len(audio_files), desc=f"Processing {source_name}") as pbar:
    ...
    def train_models(self, df_results):
        ...
        # Prepare features
        metadata_columns = ['filename', 'full_path', 'source_directory', 'file_extension', 'is_ai', 'spectrogram_path']
        self.feature_columns = [col for col in df_results.columns if col not in metadata_columns]
        ...
        session_info = {
            ...
            'test_accuracies': {name: info['test_accuracy'] for name, info in results.items()},
    ...
    def update_with_new_data(self, df_new, retrain_batch_models=False):
        ...
        # Handle new features
        metadata_columns = ['filename', 'full_path', 'source_directory', 'file_extension', 'is_ai', 'spectrogram_path']
        new_feature_cols = [col for col in df_new.columns if col not in metadata_columns]
        ...
        if retrain_batch_models and len(unique_classes) < 2:
            print(f"WARNING: New data only contains {len(unique_classes)} class(es). Cannot retrain batch models.")
            print(f"Available classes: {unique_classes}")
            print("TIP: For batch model retraining, you need both AI and Human samples in the new data.")
            retrain_batch_models = False
        
        # Transform with existing scaler
        X_new_scaled = self.scaler.transform(X_new)
        
        print(f"Updating with {len(y_new)} samples ({y_new.sum()} AI, {(~y_new.astype(bool)).sum()} Human)")
                ...
                for name, model in self.batch_models.items():
                    try:
                        ...
                        if len(X_new_scaled) < 2:
                            print(f"Not enough samples to retrain {name} (need at least 2)")
                            continue
                        ...
                        if len(available_classes) < 2:
                            print(f"Only {len(available_classes)} class(es) available for {name}, skipping")
                            continue
                        ...
                        if name == 'GradientBoosting':
                            if len(X_new_scaled) < 10:
                                print(f"Warning: Small sample size ({len(X_new_scaled)}) for {name}")
                            
                            if min_class_size < 2:
                                print(f"Insufficient samples in minority class for {name} (min: {min_class_size})")
                                print("TIP: GradientBoosting needs at least 2 samples per class for retraining")
                                continue
                        ...
                        try:
                            if name == 'RandomForest':
                                self.batch_models[name] = RandomForestClassifier(n_estimators=200, random_state=42, n_jobs=-1)
                            elif name == 'GradientBoosting':
                                self.batch_models[name] = GradientBoostingClassifier(n_estimators=200, random_state=42)
                            print(f"Reinitialized {name} model (will use previous training)")
    ...
    def predict_batch(self, audio_dir, output_file=None, max_workers=None, use_multiprocessing=True):
        ...
        if results:
            ...
            print(f"  Human files: {human_count} ({100*human_count/len(df_results):.1f}%)")
            ...
            if len(uncertain_files) > 0:
                print(f"  Uncertain predictions: {len(uncertain_files)} ({100*len(uncertain_files)/len(df_results):.1f}%)")
    ...
    def _predict_batch_multiprocessing(self, audio_files, max_workers):
        ...
        args_list = [(audio_file, self.feature_columns, scaler_data, all_models) 
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            # Submit all jobs
            future_to_file = {executor.submit(process_single_prediction, args): args[0] 
                             for args in args_list}
        ...
        print(f"Parallel prediction complete: {successful_files} success, {failed_files} failed")
        return results
        ...
        if model_file.exists():
            try:
                ...
                if 'incremental_models' in model_data:
                    ...
                    self.incremental_models = {k: v for k, v in old_models.items() 
                                             if k in ['SGD', 'PassiveAggressive']}
                    self.batch_models = {k: v for k, v in old_models.items() 
                                       if k in ['RandomForest', 'GradientBoosting']}
                
                # Validate loaded models
                for name, model in self.incremental_models.items():
                    if model is None:
                        print(f"WARNING: Incremental model {name} is None, reinitializing")
                        if name == 'SGD':
                            self.incremental_models[name] = SGDClassifier(random_state=42, loss='log_loss')
                        elif name == 'PassiveAggressive':
                            self.incremental_models[name] = PassiveAggressiveClassifier(random_state=42)
    ...
    def create_visualizations(self, df_results, save_dir=None):
        ...
        if 'audio_duration' in df_results.columns:
            ...
            ax2.hist(human_durations, alpha=0.7, label='Human', bins=20, color='lightblue')
        ...
        ax3 = axes[1, 0]
        if 'RandomForest' in self.batch_models and hasattr(self.batch_models['RandomForest'], 'feature_importances_'):
            ...
            ax3.set_yticklabels([name[:20] for name in feature_names])  # Truncate long names
        ...
        if self.training_history:
            ...
            for i, session in enumerate(self.training_history):
                ...
                if 'test_accuracies' in session:
                    ...
                    best_acc = max([v['accuracy'] for v in session['update_accuracies'].values()] + [0])
                else:
                    best_acc = 0.5
    ...
    def add_ai_data(self, ai_audio_dir, retrain_batch_models=False):
        ...
        ai_features = self.extract_features_from_directory(ai_audio_dir, is_ai_directory=True)
    ...
    def add_human_data(self, human_audio_dir, retrain_batch_models=False):
        ...
        human_features = self.extract_features_from_directory(human_audio_dir, is_ai_directory=False)
    ...
    def add_mixed_data_batch(self, data_directories, retrain_batch_models=False):
        ...
        for dir_info in data_directories:
            ...
            features = self.extract_features_from_directory(dir_path, is_ai_directory=is_ai_dir)
    ...
    def show_data_balance(self):
        ...
        for i, session in enumerate(self.training_history):
            ...
            print(f"  Session {i+1} ({session_type}): AI={ai_samples}, Human={human_samples}")
    ...
    def generate_spectrograms_batch(self, audio_dir, output_dir, spectrogram_type='mel',
                                  max_workers=4, figsize=(12, 8), dpi=300):
        """
        ...
        print(f"Generating {spectrogram_type} spectrograms for {len(audio_files)} files...")
        ...
        if len(audio_files) > 3 and max_workers > 1:
            ...
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                future_to_file = {executor.submit(generate_func, audio_file): audio_file 
                                 for audio_file in audio_files}
                
                with tqdm(total=len(audio_files), desc=f"Generating {spectrogram_type} spectrograms") as pbar:
                    for future in as_completed(future_to_file):
                        try:
                            ...
                            audio_file = future_to_file[future]
                            print(f"Error generating spectrogram for {audio_file.name}: {e}")
                        pbar.update(1)
        else:
            # Sequential processing for small batches
            for audio_file in tqdm(audio_files, desc=f"Generating {spectrogram_type} spectrograms"):
    ...
    @staticmethod
    def _generate_single_spectrogram(audio_file, output_dir, spectrogram_type, figsize, dpi):
        """Helper function for parallel spectrogram generation."""
        try:
            analyzer = AudioAnalyzer()
            return analyzer.save_spectrogram(audio_file, output_dir, spectrogram_type, figsize, dpi)
        except Exception as e:
            print(f"Error in parallel spectrogram generation for {audio_file.name}: {e}")
            return None
    
    def create_spectrogram_comparison(self, ai_dir, human_dir, output_dir, 
                                    spectrogram_type='mel', num_samples=5):
        """
        ...
        # Find audio files
        audio_extensions = ('.wav', '.mp3', '.flac', '.ogg', '.m4a', '.aac')
        ai_files = [f for f in ai_path.iterdir() if f.suffix.lower() in audio_extensions]
        human_files = [f for f in human_path.iterdir() if f.suffix.lower() in audio_extensions]
        ...
        for i, (ai_file, human_file) in enumerate(zip(ai_sample, human_sample)):
            try:
                ...
                if spectrogram_type.lower() == 'mel':
                    ...
                    S_human = librosa.feature.melspectrogram(y=y_human, sr=sr_human, n_mels=128)
                    S_human_dB = librosa.power_to_db(S_human, ref=np.max)
                    
                    img1 = librosa.display.specshow(S_ai_dB, x_axis='time', y_axis='mel', 
                                                  sr=sr_ai, fmax=8000, ax=ax1)
                    img2 = librosa.display.specshow(S_human_dB, x_axis='time', y_axis='mel', 
                                                  sr=sr_human, fmax=8000, ax=ax2)
                    ...
                    img1 = librosa.display.specshow(S_ai_dB, x_axis='time', y_axis='hz', 
                                                  sr=sr_ai, ax=ax1)
                    img2 = librosa.display.specshow(S_human_dB, x_axis='time', y_axis='hz', 
                                                  sr=sr_human, ax=ax2)
...
Examples:
  ...
  python ai_audio_detector.py --train --human-dir /path/to/human --ai-dir /path/to/ai
    ...
    # Prediction options
    parser.add_argument('--predict-file', '-f', type=str,
                       help='Predict a single audio file')
    parser.add_argument('--predict-batch', '-b', type=str,
                       help='Predict all files in a directory')
    ...
    parser.add_argument('--human-dir', type=str,
                       help='Directory containing human-generated audio')
    parser.add_argument('--ai-dir', type=str,
                       help='Directory containing AI-generated audio')
        ...
        if args.predict_file:
            if detector.load_models():
                ...
                if result:
                    ...
                    print(f"Prediction: {'AI GENERATED' if result['is_ai'] else 'HUMAN GENERATED'}")
            ...
            if detector.load_models():
                ...
                results = detector.predict_batch(args.predict_batch, args.output, max_workers, use_mp)
                if not results:
                    sys.exit(1)
            else:
                print("No trained models found. Train models first.")
            ...
            human_features = detector.extract_features_from_directory(args.human_dir, is_ai_directory=False)
            ai_features = detector.extract_features_from_directory(args.ai_dir, is_ai_directory=True)
            ...
            print(f"Training complete! Best model: {best_model[0]} (accuracy: {best_model[1]['test_accuracy']:.3f})")
...
def run_interactive_mode(detector):
    ...
    if choice == '1':
        ...
        save_spectrograms = input("\nSave spectrograms during training? (y/n): ").strip().lower() == 'y'
        spectrogram_dir = None
        spectrogram_type = 'mel'
        
        if save_spectrograms:
            spectrogram_dir = input("Enter directory to save spectrograms (or Enter for default): ").strip()
        ...
        human_features = detector.extract_features_from_directory(
            human_dir, is_ai_directory=False, save_spectrograms=save_spectrograms,
            spectrogram_dir=spectrogram_dir, spectrogram_type=spectrogram_type
        )
        ai_features = detector.extract_features_from_directory(
            ai_dir, is_ai_directory=True, save_spectrograms=save_spectrograms,
            spectrogram_dir=spectrogram_dir, spectrogram_type=spectrogram_type
        )
        ...
        print(f"\nTRAINING COMPLETE!")
        best_model = max(training_results.items(), key=lambda x: x[1]['test_accuracy'])
        print(f"Best model: {best_model[0]} (accuracy: {best_model[1]['test_accuracy']:.3f})")
            ...
            if result:
                ...
                if 'individual_predictions' in result:
                    ...
                    for model_name, pred_info in result['individual_predictions'].items():
                        ...
                        print(f"  {indicator} {model_name}: {pred_info['ai_probability']:.3f} (conf: {pred_info['confidence']:.3f})")
            else:
                print("Could not analyze file")
        ...
        if detector.load_models():
            ...
            max_workers = input("Max workers for multiprocessing (or Enter for 4): ").strip()
        ...
        if detector.load_models():
            ...
            new_dir = input("Enter directory with new audio files: ").strip().strip('"\'')
            ...
            if is_ai is not None:
                new_features = detector.extract_features_from_directory(new_dir, is_ai_directory=is_ai)
            else:
                # For mixed data, analyze each file individually
                new_features = []
                audio_extensions = detector.get_audio_extensions()
                audio_files = [f for f in Path(new_dir).iterdir() if f.suffix.lower() in audio_extensions]
                
                for audio_file in tqdm(audio_files, desc="Processing mixed data"):
                    # Check filename for AI indicators
                    keywords = input("Enter keywords to identify AI files (comma-separated): ").strip().split(',')
            ...
            if new_features:
                ...
                # Update models
                update_results = detector.update_with_new_data(df_new, retrain_batch_models=retrain_batch)
        ...
        # Add AI data only
        if detector.load_models():
            ai_dir = input("Enter path to NEW AI audio directory: ").strip().strip('"\'')
            if Path(ai_dir).exists():
                retrain_batch = input("Retrain batch models too? (y/n): ").strip().lower() == 'y'
        ...
        # Add Human data only
        if detector.load_models():
            human_dir = input("Enter path to NEW Human audio directory: ").strip().strip('"\'')
        ...
        if detector.load_models():
            ...
            print("Enter directory paths and labels. Press Enter with empty path to finish.")
            
            directories = []
            while True:
                dir_path = input(f"\nDirectory path #{len(directories)+1} (or Enter to finish): ").strip().strip('"\'')
                if not dir_path:
                    break
            ...
            if directories:
                ...
                if confirm:
                    retrain_batch = input("Retrain batch models too? (y/n): ").strip().lower() == 'y'
                    detector.add_mixed_data_batch(directories, retrain_batch_models=retrain_batch)
            else:
        ...
        if detector.load_models():
            ...
            if not detector.training_history:
                ...
                for i, session in enumerate(detector.training_history):
                    ...
                    print(f"  Samples: {session['total_samples']} (AI: {session['ai_samples']}, Human: {session['human_samples']})")
                    ...
                    if 'test_accuracies' in session:
                        ...
                        if session['update_accuracies']:
                            best_acc = max([v['accuracy'] for v in session['update_accuracies'].values()])
        ...
        output_dir = input("Enter output directory for spectrograms: ").strip().strip('"\'')
        if not output_dir:
        ...
        max_workers = input("Max workers for parallel processing (or Enter for 4): ").strip()
        if max_workers.isdigit():
            max_workers = int(max_workers)
        else:
            max_workers = 4
        ...
        output_dir = input("Enter output directory for comparisons: ").strip().strip('"\'')
        if not output_dir:
            output_dir = detector.base_dir / detector.config['output']['comparisons_dir']
        else:
            output_dir = Path(output_dir)
