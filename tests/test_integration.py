...
class TestIntegration(unittest.TestCase):
    ...
    def create_sample_audio_files(self) -> None:
        ...
        for i in range(3):
            ...
            freq2 = 880 + i * 150
            audio = (
                np.sin(2 * np.pi * freq1 * t) + 0.5 * np.sin(2 * np.pi * freq2 * t) + 0.1 * np.random.normal(0, 1, len(t))
            )  # Add some noise
    ...
    def test_end_to_end_workflow(self) -> None:
        ...
        prediction_result = self.detector.predict_single_file(test_file)
        ...
        batch_results = self.detector.predict_batch(self.ai_audio_dir)
        ...
        new_features = self.detector.extract_features_from_directory(self.ai_audio_dir, is_ai_directory=True)

        # Filter to just the new file
        new_df = pd.DataFrame([f for f in new_features if f["filename"] == "new_ai_sample.wav"])
    ...
    def test_audio_analyzer_integration(self) -> None:
        ...
        compression_features = [k for k in features.keys() if k.startswith("compression_")]
    ...
    def test_spectrogram_generation(self) -> None:
        ...
        spectrogram_success = self.detector.generate_spectrogram(test_file)
    ...
    def test_model_persistence_integration(self) -> None:
        ...
        test_file = list(self.ai_audio_dir.glob("*.wav"))[0]
        prediction = new_detector.predict_single_file(test_file)
