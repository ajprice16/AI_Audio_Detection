...
def example_training():
    ...
    human_features = detector.extract_features_from_directory(
        human_dir, is_ai_directory=False
    )
...
def example_prediction(detector=None):
    ...
    if Path(test_dir).exists():
        ...
        if results is not None:
            ...
            print(
                f"  Human files: {total_count-ai_count} ({100*(total_count-ai_count)/total_count:.1f}%)"
            )
...
def example_adaptive_learning(detector=None):
    ...
    if Path(new_human_dir).exists():
        ...
        update_results = detector.add_human_data(
            new_human_dir, retrain_batch_models=True
        )
